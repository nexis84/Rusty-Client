...
Write-Host "Copying application files (this may take a moment)..." -ForegroundColor Yellow

$filesToCopy = Get-ChildItem -Path "dist\Main.dist" -Recurse
$totalFiles = $filesToCopy.Count
$copiedFiles = 0
$skippedFiles = 0

foreach ($item in $filesToCopy) {
    $relativePath = $item.FullName.Substring((Resolve-Path "dist\Main.dist").Path.Length + 1)
    $destination = Join-Path $appFolder $relativePath
...
if (Test-Path "$appFolder\Main.exe") {
    try {
        Rename-Item -Path "$appFolder\Main.exe" -NewName "RustyBot.exe" -ErrorAction Stop
        Write-Host "Renamed Main.exe to RustyBot.exe" -ForegroundColor Green
    }
    catch {
        Write-Host "Warning: Could not rename Main.exe (may be running)" -ForegroundColor Yellow
    }
}
